...
{
    /// <summary>
    /// Demonstrates how to use SQLite storage with the graph
    /// </summary>
    public static async Task RunSqliteExampleAsync()
    {
        Console.WriteLine("=== SQLITE GRAPH STORAGE EXAMPLE ===\n");

        try
        {
            // Step 0: Clean up any existing database to avoid locks
            var dbPath = "example_graph.db";
            if (File.Exists(dbPath))
            {
                Console.WriteLine("0. Cleaning up existing database...");
                try
                {
                    File.Delete(dbPath);
                ...
                {
                    Console.WriteLine($"   Warning: Could not delete {dbPath}: {ex.Message}");
                    // Try a different filename if the original is locked
                    dbPath = $"example_graph_{DateTime.Now:yyyyMMdd_HHmmss}.db";
                    Console.WriteLine($"   Using alternative name: {dbPath}");
                }
            }

            // Step 1: Create a sample graph
            Console.WriteLine("1. Creating sample graph...");
            var graph = await CreateSampleGraphAsync();
            
            // Step 2: Save to SQLite
            Console.WriteLine("\n2. Saving graph to SQLite database...");
            await graph.SaveWithStatusAsync(dbPath);
            
            // Step 3: Clear the in-memory graph
            Console.WriteLine("\n3. Clearing in-memory graph...");
            graph.Clear();
            Console.WriteLine($"   Graph cleared: {graph.EntityCount} entities, {graph.RelationshipCount} relationships");
            
            // Step 4: Load from SQLite
            Console.WriteLine("\n4. Loading graph from SQLite database...");
            var loadedGraph = await GraphStore.LoadWithStatusAsync(dbPath);
            
            // Step 5: Verify the loaded data
            Console.WriteLine("\n5. Verifying loaded data...");
            VerifyLoadedGraph(loadedGraph);
            
            // Step 6: Demonstrate search functionality
            Console.WriteLine("\n6. Demonstrating search functionality...");
            await DemonstrateSearchAsync(dbPath);
            
            // Step 7: Show database statistics
            Console.WriteLine("\n7. Database statistics...");
            var (entityCount, relationshipCount, lastSaved) = await GraphStore.GetDatabaseStatisticsAsync(dbPath);
            Console.WriteLine($"   Database contains {entityCount} entities and {relationshipCount} relationships");
            Console.WriteLine($"   Last saved: {lastSaved:yyyy-MM-dd HH:mm:ss} UTC");
            
            // Step 8: Demonstrate community analysis on loaded graph
            Console.WriteLine("\n8. Running community analysis on loaded graph...");
            if (loadedGraph.EntityCount > 0)
            {
                using var output = Program.ui.BeginRealtime("Community Analysis");
                loadedGraph.PrintCommunitySummaryTable(output);
            }
            
            Console.WriteLine("\n✅ Example completed successfully!");
            Console.WriteLine($"\nDatabase saved as: {System.IO.Path.GetFullPath(dbPath)}");
            Console.WriteLine("You can inspect the SQLite database using tools like DB Browser for SQLite.");
    ...
    private static async Task<GraphStore> CreateSampleGraphAsync()
    {
        var graph = new GraphStore();
        
        // Add people
        graph.AddEntity(new Entity("Alice Johnson", "Person", "Senior Software Engineer with expertise in machine learning", "example"));
        graph.AddEntity(new Entity("Bob Smith", "Person", "Product Manager focusing on AI products", "example"));
        graph.AddEntity(new Entity("Carol Davis", "Person", "Data Scientist specializing in natural language processing", "example"));
        graph.AddEntity(new Entity("David Wilson", "Person", "DevOps Engineer managing cloud infrastructure", "example"));
        
        // Add organizations
        graph.AddEntity(new Entity("TechCorp", "Organization", "Leading technology company specializing in AI solutions", "example"));
        graph.AddEntity(new Entity("DataSystems Inc", "Organization", "Data infrastructure and analytics company", "example"));
        
        // Add projects
        graph.AddEntity(new Entity("Project Alpha", "Project", "Machine learning platform for recommendation systems", "example"));
        graph.AddEntity(new Entity("Project Beta", "Project", "Natural language processing chatbot system", "example"));
        graph.AddEntity(new Entity("Project Gamma", "Project", "Cloud infrastructure automation platform", "example"));
        
        // Add technologies
        graph.AddEntity(new Entity("Python", "Technology", "Programming language widely used in data science and AI", "example"));
        graph.AddEntity(new Entity("Docker", "Technology", "Containerization platform for application deployment", "example"));
        graph.AddEntity(new Entity("Kubernetes", "Technology", "Container orchestration system for scalable deployments", "example"));
        graph.AddEntity(new Entity("TensorFlow", "Technology", "Open-source machine learning framework", "example"));
        
        // Add employment relationships
        graph.AddRelationship("Alice Johnson", "TechCorp", "works_for", "Senior Software Engineer since 2020");
        graph.AddRelationship("Bob Smith", "TechCorp", "works_for", "Product Manager since 2019");
        graph.AddRelationship("Carol Davis", "DataSystems Inc", "works_for", "Principal Data Scientist since 2021");
        graph.AddRelationship("David Wilson", "TechCorp", "works_for", "DevOps Lead since 2018");
        
        // Add project assignments
        graph.AddRelationship("Alice Johnson", "Project Alpha", "leads", "Technical lead responsible for ML model development");
        graph.AddRelationship("Bob Smith", "Project Alpha", "manages", "Product owner and stakeholder coordinator");
        graph.AddRelationship("Carol Davis", "Project Beta", "leads", "NLP research and implementation lead");
        graph.AddRelationship("David Wilson", "Project Gamma", "leads", "Infrastructure architecture and automation lead");
        
        // Add technology usage
        graph.AddRelationship("Alice Johnson", "Python", "uses", "Primary development language for ML projects");
        graph.AddRelationship("Alice Johnson", "TensorFlow", "uses", "Framework for building and training ML models");
        graph.AddRelationship("Carol Davis", "Python", "uses", "Data analysis and NLP model development");
        graph.AddRelationship("David Wilson", "Docker", "uses", "Container technology for application packaging");
        graph.AddRelationship("David Wilson", "Kubernetes", "uses", "Orchestration platform for production deployments");
        
        // Add project technology dependencies
        graph.AddRelationship("Project Alpha", "TensorFlow", "depends_on", "Core ML framework for recommendation algorithms");
        graph.AddRelationship("Project Alpha", "Python", "built_with", "Primary development language");
        graph.AddRelationship("Project Beta", "Python", "built_with", "NLP processing and model serving");
        graph.AddRelationship("Project Gamma", "Docker", "uses", "Containerization for infrastructure tools");
        graph.AddRelationship("Project Gamma", "Kubernetes", "uses", "Deployment and scaling platform");
        
        // Add collaboration relationships
        graph.AddRelationship("Alice Johnson", "Carol Davis", "collaborates_with", "Joint research on ML and NLP integration");
        graph.AddRelationship("Bob Smith", "David Wilson", "coordinates_with", "Product and infrastructure alignment");
        
        Console.WriteLine($"   Created graph with {graph.EntityCount} entities and {graph.RelationshipCount} relationships");
        
        return graph;
    }

    private static void VerifyLoadedGraph(GraphStore graph)
    {
        Console.WriteLine($"   Loaded graph contains {graph.EntityCount} entities and {graph.RelationshipCount} relationships");
        
        // Verify specific entities
        if (!graph.Entities.ContainsKey("Alice Johnson"))
            throw new Exception("Alice Johnson entity not found");
            
        if (!graph.Entities.ContainsKey("TechCorp"))
            throw new Exception("TechCorp entity not found");
        
        // Verify relationships
        var aliceConnections = graph.GetConnectedEntities("Alice Johnson");
        Console.WriteLine($"   Alice Johnson is connected to {aliceConnections.Count} other entities:");
        foreach (var connection in aliceConnections.Take(3))
        {
            Console.WriteLine($"     - {connection.Name} ({connection.Type})");
        }
        
        Console.WriteLine("   ✓ Data verification completed successfully");
    }

    private static async Task DemonstrateSearchAsync(string dbPath)
    {
        // Search for people
        var people = await GraphStore.SearchEntitiesInDatabaseAsync("", dbPath, "Person");
        Console.WriteLine($"   Found {people.Count} people in the database:");
        foreach (var person in people)
        {
            Console.WriteLine($"     - {person.Name}: {person.Attributes}");
        }
        
        // Search for entities containing "Project"
        var projects = await GraphStore.SearchEntitiesInDatabaseAsync("Project", dbPath);
        Console.WriteLine($"\n   Found {projects.Count} entities containing 'Project':");
        ...
        var technologies = await GraphStore.SearchEntitiesInDatabaseAsync("", dbPath, "Technology");
        Console.WriteLine($"\n   Found {technologies.Count} technologies:");
        foreach (var tech in technologies)
        {
            Console.WriteLine($"     - {tech.Name}: {tech.Attributes}");
        }
    }
